...
|---|---|---|---:|---:|
| Master | redis-01 | 10.10.10.11 | 6379 | 26379 |
| Replica | redis-02 | 10.10.10.12 | 6379 | 26379 |
| Replica | redis-03 | 10.10.10.13 | 6379 | 26379 |
...
```bash
nc -vz 10.10.10.11 6379
nc -vz 10.10.10.12 6379
nc -vz 10.10.10.13 6379
...
Redis `6379` 仅允许应用服务器、Redis 节点、监控服务器访问。
...
firewall-cmd --permanent --zone=redis-ha --add-port=6379/tcp
...
redis-cli -h 10.10.10.11 -p 6379 --user monitor INFO replication
unset REDISCLI_AUTH
```
...
```bash
export REDISCLI_AUTH='ReplaceWith_Monitor_StrongPassword_2026'
redis-cli -h 10.10.10.12 -p 6379 --user monitor INFO replication
unset REDISCLI_AUTH
```
...
redis-cli -h 10.10.10.11 -p 6379 --user app SET ha:check ok EX 300
unset REDISCLI_AUTH
```
...
```bash
export REDISCLI_AUTH='ReplaceWith_App_StrongPassword_2026'
redis-cli -h 10.10.10.12 -p 6379 --user app GET ha:check
redis-cli -h 10.10.10.13 -p 6379 --user app GET ha:check
unset REDISCLI_AUTH
```
...
redis-cli -h 10.10.10.12 -p 6379 --user monitor INFO replication
redis-cli -h 10.10.10.13 -p 6379 --user monitor INFO replication
```
