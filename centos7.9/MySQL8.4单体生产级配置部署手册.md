...
```text
/data/module/                                      # 安装包与模块目录
/data/module/mysql-8.4.9-linux-glibc2.17-x86_64/  # 具体版本目录
/data/module/mysql                                # 当前版本软链接
/data/mysql/13306/                                 # 实例数据、日志、临时文件目录
/etc/my.cnf                                       # 主配置文件
/etc/systemd/system/mysqld.service                # systemd 服务文件
```

> 说明：生产环境推荐保留带版本号的真实目录，并通过 `/data/module/mysql` 软链接暴露当前版本。不建议将解压目录直接改名为 `mysql`，也不建议解压时使用 `--strip-components` 直接覆盖到固定目录。这样可以保留版本信息，便于升级、回滚、资产登记和故障排查；systemd、环境变量和脚本只引用 `/data/module/mysql`，升级时通常只需要切换软链接。
...
| 数据目录 | `/data/mysql/13306/data` | InnoDB 数据文件与系统表空间 |
| binlog 目录 | `/data/mysql/13306/binlog` | 二进制日志 |
| relaylog 目录 | `/data/mysql/13306/relaylog` | 后续复制预留 |
| undo 目录 | `/data/mysql/13306/undo` | undo tablespace |
| 临时目录 | `/data/mysql/13306/tmp` | MySQL 临时文件 |
| 日志目录 | `/data/mysql/13306/log` | error log、slow log、general log |
| socket 目录 | `/data/mysql/13306/run` | socket 与 pid 文件 |
| 备份目录 | `/data/backup/mysql` | 本机临时备份落盘目录 |

创建目录：

```bash
mkdir -p /data/mysql/13306/{data,binlog,relaylog,undo,tmp,log,run}
...
chmod 750 /data/mysql/13306
chmod 750 /data/mysql/13306/{data,binlog,relaylog,undo,tmp,log,run}
...
ls -ld /data/mysql/13306 /data/mysql/13306/* /data/backup/mysql
```
...
[client]
port = 13306
socket = /data/mysql/13306/run/mysql.sock
...
datadir = /data/mysql/13306/data
socket = /data/mysql/13306/run/mysql.sock
pid-file = /data/mysql/13306/run/mysqld.pid
tmpdir = /data/mysql/13306/tmp
...
innodb_undo_directory = /data/mysql/13306/undo
...
# binlog
log_bin = /data/mysql/13306/binlog/mysql-bin
binlog_format = ROW
...
relay_log = /data/mysql/13306/relaylog/mysql-relay-bin
relay_log_recovery = 1

# logs
log_error = /data/mysql/13306/log/error.log
slow_query_log = 1
slow_query_log_file = /data/mysql/13306/log/slow.log
long_query_time = 1
log_queries_not_using_indexes = 0
log_slow_admin_statements = 1
log_slow_replica_statements = 1
general_log = 0
general_log_file = /data/mysql/13306/log/general.log
...
skip_name_resolve = 1
symbolic-links = 0
secure_file_priv = /data/mysql/13306/tmp
...
- `/etc/my.cnf` 中 `[client]` 和 `[mysqld]` 的 `port`；
...
```bash
ls -la /data/mysql/13306/data
ls -la /data/mysql/13306/undo
```
...
```bash
grep 'temporary password' /data/mysql/13306/log/error.log
```
...
```bash
tail -100 /data/mysql/13306/log/error.log
ss -lntp | grep ':13306'
```
...
```bash
mysql -uroot -p -S /data/mysql/13306/run/mysql.sock
```
...
```bash
ls -lh /data/mysql/13306/log/
ls -lh /data/mysql/13306/binlog/
tail -50 /data/mysql/13306/log/error.log
```
...
systemctl restart mysqld
systemctl is-active mysqld
mysqladmin -uroot -p -S /data/mysql/13306/run/mysql.sock ping
```
...
```bash
tail -100 /data/mysql/13306/log/slow.log
```
...
find /data/mysql/13306/binlog -type f -name 'mysql-bin.*' -printf '%TY-%Tm-%Td %TH:%TM %s %p\n' | sort | tail -20
```
